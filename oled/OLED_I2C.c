...
void OLED_RenderPage(Page *page) {
    ...
    for (int i = 0 ;i < count_of(page->lines); i++) {
        OLED_WriteString(OLED_STRING_OFFSET, line, page->lines[i]);
        line++;
    }
...
void OLED_InsertCursor(Page *page) {
    ...
    for (int line = page->first_opt + 1; line <= page->last_opt + 1; line++) {
        if (page->selected + 1 == line)
